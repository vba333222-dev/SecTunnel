...
class CookieManagerService {
  ...
  static Database? _sessionDbCache;

  static Future<Database> _getSessionDb(String userDataFolder) async {
    final dbPath = getSessionDbPath(userDataFolder);
    
    if (_sessionDbCache != null) {
      return _sessionDbCache!;
    }
    
    _sessionDbCache = await openDatabase(
      dbPath,
      version: 1,
  ...
  static Future<void> saveSessionToDb(String userDataFolder, List<CookieItem> cookies) async {
    ...
    final db = await _getSessionDb(userDataFolder);
  ...
  static Future<List<CookieItem>> loadSessionFromDb(String userDataFolder) async {
    ...
    try {
      final results = await db.query('cookies');
  ...
  static Future<List<CookieItem>> exportCookies(String profileId) async {
    ...
    try {
      ...
      final appDataDir = supportDir.parent;
